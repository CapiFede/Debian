#!/usr/bin/env bash
#
# backup.sh — Respalda mis perfiles de Helium (+ lanzadores) al drive Other.
# ---------------------------------------------------------------------------
...
# (marcadores, logins, extensiones, preferencias, historial) a /mnt/Other/Backup,
...
#
# El drive Other NO se formatea al reinstalar, así que lo copiado sobrevive.
...
if [ "$warn" -eq 0 ]; then
  ...
  echo "⚠ Terminó con avisos (algún archivo no se pudo copiar; suele ser inofensivo)."
fi
